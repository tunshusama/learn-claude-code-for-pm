# s09: Context Compact — コンテキストはいつか満杯になる、場所を空ける方法が必要
...
1. `Read the file README.md, then read code.py, then read s02_agent_loop/README.md`（連続して複数のファイルを読み、L2 の古い結果圧縮を観察）
2. `Read every file in s09_context_compact/`（一度に大量の内容を読み込み、L3 のディスク退避を観察）
