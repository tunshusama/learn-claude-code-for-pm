...
Builds on s08 (skill loading). Usage:

    python s09_context_compact/code.py
...
# s08: Skill catalog scan (inherited from s08)
...
# s09: SYSTEM includes skill catalog (inherited from s08 build_system)
...
# s09: subagent gets its own system prompt — no compact, no skill loading
SUB_SYSTEM = (
    f"You are a coding agent at {WORKDIR}. "
...
# ═══════════════════════════════════════════════════════════
#  FROM s03-s08 (unchanged): Basic Tools
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
#  FROM s07-s08 (unchanged): Subagent
# ═══════════════════════════════════════════════════════════
...
TOOLS = [
    ...
    # s09 change: new compact tool — triggers compact_history, not a no-op
...
# ═══════════════════════════════════════════════════════════
#  agent_loop — s09 core: run compaction pipeline before LLM
# ═══════════════════════════════════════════════════════════
...
def agent_loop(messages: list):
    reactive_retries = 0
    while True:
        # s09 change: three preprocessors (0 API calls, cheap first)
        ...
        # s09 change: tokens still over threshold → LLM summary (1 API call)
        ...
        for block in response.content:
            ...
            # s09: compact tool triggers compact_history, not a no-op string
...
if __name__ == "__main__":
    print("s09: Context Compact — four-layer compaction pipeline")
