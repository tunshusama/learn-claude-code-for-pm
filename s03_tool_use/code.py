#!/usr/bin/env python3
"""s03: minimal tool dispatch agent loop."""
...
def run_bash(command: str) -> str:
    ...
    if any(text in command for text in dangerous):
    ...
    while True:
        ...
        for block in response.content:
            if block.type != "tool_use":
                continue
            print(f"\033[33m工具: {block.name} {dict(block.input)}\033[0m")
...
if __name__ == "__main__":
    require_setup()
    print("s03: Tool Use")
    print("输入 /exit 退出。试试让 Agent 读取 README.md、创建文件、查找 *.py。\n")
